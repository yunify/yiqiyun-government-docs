...
YKE 预安装了 NFS 客户端程序，用户对接 NFS 服务端时应确保 YKE 各节点有权限挂载 NFS 服务端文件夹。用户可以使用 Kubernetes 官方方法（参考文档：https://kubernetes.io/docs/concepts/storage/volumes/#nfs）对接 NFS 服务端，这是一种静态分配存储卷方法，分配和回收存储卷过程复杂，可对接多个 NFS 服务端。为了方便用户对接 NFS 服务端，YKE 预置了 `NFS 动态分配器`，支持动态分配存储卷，分配和回收存储卷过程简便，可对接一个 NFS 服务端。
