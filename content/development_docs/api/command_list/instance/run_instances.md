...
创建云服务器时，一旦参数 vxnets.n 包含基础网络（即： vxnet-0 ），则需要指定防火墙 security_group，如果没有指定，山东省计算中心云平台会自动使用缺省防火墙。

山东省计算中心云平台给云服务器定义了几种经典配置，可通过参数 instance_type 指定，配置列表请参考 [_Instance Types_](../../../common/instance_type/) 。 如果经典配置不能满足你的需求，可通过参数 cpu, memory 自定义云服务器配置。
...
| --- | --- | --- | --- |
| image_id | String | 镜像ID，此镜像将作为云服务器的模板。可传山东省计算中心云平台提供的镜像ID，或自己创建的镜像ID | Yes |
...
```
https://api.yiqiyun.sd.cegn.cn/iaas/?action=RunInstances
&vxnets.1=vxnet-0
