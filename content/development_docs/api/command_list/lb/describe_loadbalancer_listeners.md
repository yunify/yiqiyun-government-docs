...
| --- | --- | --- |
| action | String | 响应动作 |
| loadbalancer_listener_set | Array | JSON 格式的监听器数据列表，每项参数可见下述 **Response Item** 部分 |
...
| create_time | TimeStamp | 监听器创建时间，为UTC时间，格式可参见 ISO8601 (访问地址：http://www.w3.org/TR/NOTE-datetime)。 |
...
```
https://api.yiqiyun.sd.cegn.cn/iaas/?action=DescribeLoadBalancerListeners
