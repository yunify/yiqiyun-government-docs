...
MetaData 是山东省计算中心云平台定义的一组关于云服务器运行参数的字段，主要包括云服务器 ID，云服务器 CPU 和内存配置，操作系统，挂载的硬盘，所在的子网等。

### 1. 通过文件写入

山东省计算中心云平台可以通过将 MetaData 写入云服务器内文件的方式提供给用户，用户对文件做一定处理后即可获得 MetaData 值。

为方便用户编程时对参数值的调取，对 MetaData 文件的参数格式做了特殊规定，目前支持两种：

1.  JSON 格式：参数与参数值写成 JSON 格式，与目前山东省计算中心云平台 API 的提供方式类似。
...
| instance_type | String | 云服务器类型，为山东省计算中心云平台预定义的一些配置类型，可参考 [_Instance Types_](/development_docs/api/common/instance_type) |
...
用户打包时需从云服务器的根目录位置开始打包，如此才可包含全路径。山东省计算中心云平台系统会将文件从云服务器的根目录起解包至对应位置。
