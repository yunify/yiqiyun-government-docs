...
如下图所示，假设您在山河计算平台中的 VPC 网络使用的公网 IP 是 3.3.3.3，您自己有两个数据中心，它们对外使用的公网 IP 分别是 6.6.6.6、9.9.9.9，这三个 VPC 网络背后各连接有两个私有网络，它们将通过 L3 GRE 隧道连接在一起。
...
1. 在 VPC 详情页的**管理配置**标签页中，参照简单的对连拓扑中 [1.在山河中的操作](#1-在山河中的操作) 的部分，创建一条到 6.6.6.6 的隧道。
