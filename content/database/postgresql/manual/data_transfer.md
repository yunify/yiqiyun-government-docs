...
导出远端数据库全部数据示例：远端数据库 IP 地址为 `192.168.100.246` ，数据库名为 `shanhe`，目标文件及路径为`/tmp/pgdatabk.sql`。

```shell
pg_dump -U shanhe -h 192.168.100.246 shanhe  > /tmp/pgdatabk.sql
```
...
数据库名称默认为 `shanhe`，可直接作为导入用户账号。示例如下：
