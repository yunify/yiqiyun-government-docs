...
使用 对象存储之前，用户需创建相应的 Bucket，用于储存 Object。详细操作可参考 [创建 Bucket](/storage/object-storage/manual/console/bucket_manage/basic_opt/#创建-bucket)。
...
- 最后用 bucketService 来把文件上传到对象存储中，通过调用 putObject 方法来实现。
