...
创建 Kafka 集群前，您需要先[创建一个 Zookeeper 集群](/bigdata/zookeeper/quickstart/create_cluster/)，其他地方创建的 ZooKeeper 集群识别不出来，建议 Kafka 与 ZooKeeper 在一个私有网络/基础网络中。
...
按照实际需求配置 Kafka 参数，同时也可以配置 Kafka-manager 是否需要登陆，登录账户与密码和端口参数。

### 第七步：用户协议

阅读并同意 云平台 AppCenter 用户协议之后，点击**提交**，即可开始部署应用。
